...
Ext.grid.GridDragZone = function(grid, config){
    ...
    // prevent the default MenuAction, but don't stop propagation
 ...
 * <p>The MenuAction column can be at any index in the columns array, and a grid can have any number of
 * MenuAction columns. </p>
 */
...
//      class name x-MenuAction-col-{n}
        me.renderer = function(v, meta) {
//          Allow a configured renderer to create initial value (And set the other values in the "metadata" argument!)
            v = Ext.isFunction(cfg.renderer) ? cfg.renderer.apply(this, arguments)||'' : '';

            meta.css += ' x-MenuAction-col-cell';
            for (i = 0; i < l; i++) {
                item = items[i];
                v += '<img alt="' + (item.altText || me.altText) + '" src="' + (item.icon || Ext.BLANK_IMAGE_URL) +
                    '" class="x-MenuAction-col-icon x-MenuAction-col-' + String(i) + ' ' + (item.iconCls || '') +
